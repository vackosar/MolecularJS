...
function Particle(position, speed, forceField, weight) {
	...
	// not weight but 1 / weight
	this.weight=1/weight;
	// not speed but step or speed * t
	this.speed=speed;
	// not force but force * t^2 * 0.5 
...
function Integrator(space) {
	...
	integrate=function() {
		...
		for(var i=0; i<particles.length; i++) {
			...
			position=force.multiply(particle.weight * particle.charge)
...
function click() {
	...
	var maxKinetic = 40;
	var particlesView = document.getElementById('particlesView');
	var maxPosition = particlesView.width; 
	...
	// brownian particle
	position = new Vector(maxPosition * 1/3, maxPosition*1/3);
